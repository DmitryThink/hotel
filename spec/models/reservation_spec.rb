require 'rails_helper'

RSpec.describe Reservation, type: :model do
  context 'with different dates' do
    let!(:room) { create(:room) }
    # before(:all) do
    #
    #   @room = Room.find_or_create_by(id:1, number: 1, number_of_people: 3, type_of_room: "standart")
    #   @room2 = Room.find_or_create_by(id:2, number: 1, number_of_people: 4, type_of_room: "luxe")
    #
    #   Month.destroy_all
    #   calculate_month(@room, 600, 800, 900, 900, 600)
    #   calculate_month(@room2, 700, 1000, 1100, 1100, 700)
    #
    #   @client = Client.create!(name: "Bill", surname: "Clinton", phone_number: "123123123123", email: "asdas@sfd.com")
    #   r = Reservation.create!(date_from: "01.06.2018", date_to: "15.06.2018", room: @room, client: @client)
    #   r.update!(prepaid: true)
    # end
    #
    # def calculate_month(room, may, jun, jul, aug, sep)
    #   month_number = 5
    #   ({'May':[may,31], 'June': [jun,30], 'July': [jul,31], 'August': [aug,31], 'September': [sep,30]}).each do |month_name, pm_arr|
    #     month_params = {name: month_name, price: pm_arr.first, number: month_number, room: room, max_days: pm_arr.last}
    #
    #     (1..pm_arr.last).each do |day_number|
    #       month_params.merge!("day_#{day_number}": room.number)
    #     end
    #
    #     Month.create!(month_params)
    #     month_number += 1
    #   end
    # end

    context 'check invalid dates' do
      it 'two correct dates' do
        r = Reservation.create!(date_from: "16.06.2018", date_to: "20.06.2018", room: @room, client: @client)
        r.prepaid = true
        r.save!
        expect(r).to be_valid
      end
      it '(1) incorrect date (the same)' do
        r = Reservation.new(date_from: "01.06.2018", date_to: "15.06.2018", room: @room, client: @client)
        r.prepaid = true
        expect { r.save! }.to raise_error(RuntimeError)
      end
      it '(2) incorrect date (date_from< && date_to<)' do
        r = Reservation.new(date_from: "01.05.2018", date_to: "14.06.2018", room: @room, client: @client)
        r.prepaid = true
        expect { r.save! }.to raise_error(RuntimeError)
      end
      it '(3) incorrect date (date_from> && date_to<)' do
        r = Reservation.new(date_from: "03.05.2018", date_to: "14.06.2018", room: @room, client: @client)
        r.prepaid = true
        expect { r.save! }.to raise_error(RuntimeError)
      end
      it '(4) incorrect date (date_from< && date_to>)' do
        r = Reservation.new(date_from: "01.05.2018", date_to: "30.06.2018", room: @room, client: @client)
        r.prepaid = true
        expect { r.save! }.to raise_error(RuntimeError)
      end
      it '(5) incorrect date (date_from> && date_to>)' do
        r = Reservation.new(date_from: "07.06.2018", date_to: "30.06.2018", room: @room, client: @client)
        r.prepaid = true
        expect { r.save! }.to raise_error(RuntimeError)
      end
    end

    # context 'check room_dates' do
    #   it 'checks number of dates' do
    #     r = Reservation.create!(date_from: "01.06.2018", date_to: "15.06.2018", room: @room, client: @client)
    #     expect { r.update!(prepaid: !r.prepaid) }.to raise_error(ActiveRecord::RecordInvalid)
    #     expect(Reservation.find(r.id).room_dates.count).to eq 0
    #     expect(Reservation.find(r.id).prepaid?).to eq false
    #   end
    # end
  end
end
